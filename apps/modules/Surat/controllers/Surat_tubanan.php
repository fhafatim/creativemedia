...
class Surat_tubanan extends AUTH_Controller
{

	public function __construct()
	{
		parent::__construct();
		$this->load->model('M_surat_tubanan');
	...
	public function index()
	{
		...
		$this->loadkonten('v_tubanan/home', $data);
	}

	public function ajax_list()
	{
		$list = $this->M_surat_tubanan->get_data();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $master) {
			...
			$row[] = $master->dibuat;
			$row[] = '<a class="btn btn-sm btn-info" href="' . base_url() . 'upload/surat/tubanan/' . $master->file . '" target="_blank"> <span class="glyphicon glyphicon-eye-open"></span></a>';
			//add html for action
			$row[] =  anchor('edit-surat-tubanan/' . $master->id_surat, ' <span class="fa fa-edit"></span> ', ' class="btn btn-sm btn-primary ajaxify klik " ') . ' ' .
	...
	{
		$config['upload_path']          = './upload/surat/tubanan/';
	...
	public function Add()
	{
		...
		/*ini harus ada boss */ else {
			$data['page'] 				= "Data Surat";
			$data['judul'] 				= "Data Surat";
			$data['nomor_surat'] = $this->M_surat_tubanan->get_nomor();
			$this->loadkonten('v_tubanan/tambah', $data);
		}
	}

	public function ProsesTambahSuratTubanan()
	{
		...
		$result = $this->db->insert('tbl_surat_tubanan', $data);
	...
	public function edit($id)
	{
		...
		/*ini harus ada boss */ else {
			...
			$data['judul'] 				= "Data Surat";
			$data['datamaster'] = $id;
			$data['surat'] = $this->M_surat_tubanan->selek_surat($id);
			$this->loadkonten('v_tubanan/update', $data);
		}
	}
	...
	public function prosesUpdateSurat()
	{
		...
		$result = $this->db->update('tbl_surat_tubanan', $data, $where);
	...
	public function hapus_surat()
	{
		...
		$barang_query = $this->db->query("DELETE FROM tbl_surat_tubanan WHERE id_surat = '$id_surat'");
	...
	{
		$fileName = 'surat-tubanan-' . date('ymdhis') . '.xlsx';

		$employeeData = $this->M_surat_tubanan->get_data_export($this->input->post('table_length'));
		...
		$writer->save("upload/excel/surat-tubanan/" . $fileName);

		$output = array(
			"data" => base_url() . "upload/excel/surat-tubanan/" . $fileName,
		);
		//output to json format
		echo json_encode($output);
		// header("Content-Type: application/vnd.ms-excel");
		// redirect(base_url()."/upload/excel/surat-tubanan/".$fileName);       
	}
}
